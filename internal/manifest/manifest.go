...
// that gh-skill-pack writes into a target repository:
//
//   - .skills/manifest.yml      — declared intent (hand-curatable)
//   - .skills/manifest.lock.yml — resolved state (generated; do not hand-edit)
//
...
const (
	...
	ManifestPath = ".skills/manifest.yml"
	// LockPath is the path (relative to the repo root) of the generated lock.
	LockPath = ".skills/manifest.lock.yml"
	// SkillsDir is the root directory under which gh-skill-pack writes installed files.
	SkillsDir = ".skills"
	// ManagedBy is the marker written into the lock file so downstream tools
	// can detect that the directory is managed by gh-skill-pack.
...
type LockFile struct {
	...
	LocalHash    string `yaml:"localHash"`    // sha256 of the file as written by gh-skill-pack
	Mode         string `yaml:"mode"`         // POSIX-style octal, e.g. "0644"
}

// LoadManifest reads .skills/manifest.yml under repoRoot. Returns an empty
...
// LoadLock reads .skills/manifest.lock.yml. Returns a zero-valued lock when
...
const manifestHeader = `# .skills/manifest.yml — declared skill/agent intent.
# Edit this file by hand to declare which plugins this repo wants.
# Run "gh skill-pack sync" (or any "gh skill-pack" mutating command) to apply.
`

const lockHeader = `# .skills/manifest.lock.yml — generated by gh-skill-pack.
# Do not edit by hand: re-run "gh skill-pack" commands to regenerate.
`
